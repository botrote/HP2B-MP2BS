...
type receiveStreamI interface {
	...
	handleStreamFrame2(*wire.StreamFrame, protocol.PacketNumber) error // yunmin added
	handleResetStreamFrame(*wire.ResetStreamFrame) error
	closeForShutdown(error)
	getWindowUpdate() protocol.ByteCount
	handleFecFrame(frame *wire.FecFrame) error // yunmin added
}
...
// yunmin added
func (s *receiveStream) handleStreamFrame2(frame *wire.StreamFrame, packetNumber protocol.PacketNumber) error {
	s.mutex.Lock()
	completed, err := s.handleStreamFrameImpl2(frame, packetNumber)
	s.mutex.Unlock()
...
// yunmin added
func (s *receiveStream) handleStreamFrameImpl2(frame *wire.StreamFrame, packetNumber protocol.PacketNumber) (bool /* completed */, error) {
	maxOffset := frame.Offset + frame.DataLen()
	if err := s.flowController.UpdateHighestReceived(maxOffset, frame.Fin); err != nil {
		return false, err
	}
	...
	// yunmin added
	if fec.FecEnable {
		s.fecHandler.PushStreamFrameToDecode(frame, packetNumber)
	}
...
// yunmin added - FEC frame
func (s *receiveStream) handleFecFrame(frame *wire.FecFrame) error {
	s.mutex.Lock()
	s.fecHandler.PushFecFrameToDecode(frame)
	s.mutex.Unlock()

	if fec.FecEnable && !s.fecStarted {
		s.fecStarted = true
		go s.pushFecDecodedFrame()
	}
	return nil
}
...
// yunmin added
func (s *receiveStream) pushFecDecodedFrame() {
	...
	for {
		...
		select {
		case fin = <-s.fecHandler.FecChanFinal:
		case frame = <-s.fecHandler.FecChan:
			fec.FecLog("receive_stream.go: pushFecDecodedFrame(): frame is decoded by Raptor Decoder %d", frame.Offset)
		}

		if frame != nil {
			s.mutex.Lock()
			err := s.frameQueue.Push(frame.Data, frame.Offset, frame.PutBack)
			s.mutex.Unlock()

			if err != nil {
				fec.FecLog("receive_stream.go: pushFecDecodedFrame(): Push Error (Frame Offset=%d)", frame.Offset)
				break
			}
			s.signalRead()
		}
		...
		if fin {
			fec.FecLog("receive_stream.go: pushFecDecodedFrame(): Finish!!!")
			break
		}
	}
}
