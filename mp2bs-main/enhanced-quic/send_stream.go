...
type sendStreamI interface {
	...
	popStreamFrame(maxBytes protocol.ByteCount) (*ackhandler.Frame, bool, bool) // yunmin added
...
func newSendStream(
	...
	s := &sendStream{
		...
		version:        version,
		fecHandler:     fec.NewFecHandler(), // yunmin added
	}
...
// yunmin added - this function can return not only stream frame but also FEC frame.
// yunmin added - also this function can return the flag for fec frame
func (s *sendStream) popStreamFrame(maxBytes protocol.ByteCount) (*ackhandler.Frame, bool /* has more data to send */, bool /* FEC frame */) {
	// s.mutex.Lock()
	// f, hasMoreData, _ := s.popNewOrRetransmittedStreamFrame(maxBytes)
	// if f != nil {
	...
	// }
	// return &ackhandler.Frame{Frame: f, OnLost: s.queueRetransmission, OnAcked: s.frameAcked}, hasMoreData, false

	// yunmin added - from this line to end of function
	var f *wire.StreamFrame
	var fecFrame *wire.FecFrame

	//fec.FecLog("send_stream.go: popStreamFrame(): sendWindow=%d", s.flowController.SendWindowSize())

	s.mutex.Lock()
	f, hasMoreData, retransmission := s.popNewOrRetransmittedStreamFrame(maxBytes)
	if f != nil {
		s.numOutstandingFrames++ // for checking acked

		// Perform FEC encoding if the frame is not retransmitted stream frame
		if !retransmission && fec.FecEnable {
			// FEC Enable
			s.fecHandler.PushToEncode(f)
		}
	}

	// Pop available FEC frame
	fecFrame = nil
	if fec.FecEnable && f == nil && !retransmission && s.fecHandler.GetFecTransmissionQueueLen() > 0 {
		//s.numOutstandingFrames++
		fecFrame = s.fecHandler.PopFecFrame(maxBytes)
		if fecFrame != nil {
			s.signalWrite()
		}
	}
	...
	if f != nil && fecFrame == nil {
		if !retransmission {
			//fec.FecLog("send_stream.go: popStreamFrame(): new stream frame: %d", f.Offset)
		} else {
			//fec.FecLog("send_stream.go: popStreamFrame(): retransmitted stream frame: %d", f.Offset)
		}
		return &ackhandler.Frame{Frame: f, OnLost: s.queueRetransmission, OnAcked: s.frameAcked}, hasMoreData, false
	} else if f == nil && fecFrame != nil {
		// fec.FecLog("send_stream.go: popStreamFrame():  Fec frame: stream offset: %d, data offset: %d, data: %d, queue: %d \n",
		// 	fecFrame.StreamOffset, fecFrame.FecDataOffset, len(fecFrame.FecData), s.fecHandler.GetFecTransmissionQueueLen())
		return &ackhandler.Frame{Frame: fecFrame}, hasMoreData, true
	} else {
		//fec.FecLog("send_stream.go: popStreamFrame(): nil !!!!!!!! ")
		return nil, hasMoreData, false
	}
}
...
// yunmin added - Add the returned flag for retransmitted stream frame
func (s *sendStream) popNewOrRetransmittedStreamFrame(maxBytes protocol.ByteCount) (*wire.StreamFrame, bool /* has more data to send */, bool /* retransmitted */) {
	...
	// yunmin added - if there is a fec frame is in fec transmission queue, return to callee (do not pop new stream frame)
	if fec.FecEnable && s.fecHandler.GetFecTransmissionQueueLen() > 0 {
	...
	if len(s.dataForWriting) == 0 && s.nextFrame == nil {
		if s.finishedWriting && !s.finSent {
			s.finSent = true
			// fec.FecLog("send_stream.popNewOrRetransmittedStreamFrame(): FIN is set (1) !!!!")
	...
	if f.Fin {
		fec.FecLog("send_stream.go: popNewOrRetransmittedStreamFrame(): FIN is set (2) !!!!")
		s.finSent = true
	}
...
func (s *sendStream) Close() error {
	...
	// fec.FecLog("send_stream.Close(): Close the stream!!")
