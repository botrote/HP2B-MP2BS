...
func (s *session) preSetup() {
	// yunmin added
	fec.FecEnable = s.config.FecEnable
	util.Log("fec.FecEnable=%v, s.config.FecEnable=%v \n", fec.FecEnable, s.config.FecEnable)
...
func (s *session) handleUnpackedPacket(
	...
	for {
		...
		if s.tracer == nil {
			...
			if err := s.handleFrame2(frame, packet.packetNumber, packet.encryptionLevel, packet.hdr.DestConnectionID); err != nil {
				return err
			}
		} else {
...
func (s *session) handleFrame(f wire.Frame, encLevel protocol.EncryptionLevel, destConnID protocol.ConnectionID) error {
	...
	case *wire.FecFrame: // yunmin added - FEC
		err = s.handleFecFrame(frame)
	default:
...
// yunmin added
func (s *session) handleFrame2(f wire.Frame, packetNumber protocol.PacketNumber, encLevel protocol.EncryptionLevel, destConnID protocol.ConnectionID) error {
	var err error
	wire.LogFrame(s.logger, f, false)
	switch frame := f.(type) {
	case *wire.CryptoFrame:
		err = s.handleCryptoFrame(frame, encLevel)
	case *wire.StreamFrame:
		err = s.handleStreamFrame2(frame, packetNumber)
	case *wire.AckFrame:
		err = s.handleAckFrame(frame, encLevel)
	case *wire.ConnectionCloseFrame:
		s.handleConnectionCloseFrame(frame)
...
// yunmin added
func (s *session) handleStreamFrame2(frame *wire.StreamFrame, packetNumber protocol.PacketNumber) error {
	str, err := s.streamsMap.GetOrOpenReceiveStream(frame.StreamID)
	if err != nil {
		return err
	}
	...
	return str.handleStreamFrame2(frame, packetNumber)
}
...
// yunmin added - FEC
func (s *session) handleFecFrame(frame *wire.FecFrame) error {
	str, err := s.streamsMap.GetOrOpenReceiveStream(frame.StreamID)
	if err != nil {
		return err
	}
	...
	return str.handleFecFrame(frame)
}
...
func (s *session) sendPackets() error {
	...
	for {
		...
		//fmt.Printf("yunmin - session.go: sendPackets(): sendMode = %d \n", sendMode)
