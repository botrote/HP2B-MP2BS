...
func newSentPacketHandler(
	...
	return &sentPacketHandler{
		...
		appDataPackets:                 newPacketNumberSpace(0, false, rttStats), // yunmin added - change true to false
		rttStats:                       rttStats,
...
func (h *sentPacketHandler) detectLostPackets(now time.Time, encLevel protocol.EncryptionLevel) error {
	...
	var lostSendTime time.Time
	if fec.FecEnable {
		lostSendTime = now.Add(-lossDelay)
	} else {
		lostSendTime = now.Add(-lossDelay - time.Duration(maxRTT)) // yunmin added
	}

	priorInFlight := h.bytesInFlight

	// yunmin added
	lostPackets := make([]*Packet, 0)
	...
	err := pnSpace.history.Iterate(func(p *Packet) (bool, error) {
		...
		if p.SendTime.Before(lostSendTime) {
			packetLost = true
			// fec.FecLog("sent_packet_handler.go: detectLostPackets(): Case#1 %d is lost (lostSendTime=%d, sendTime=%d)",
			// 	p.PacketNumber, lostSendTime.Nanosecond(), p.SendTime.Nanosecond())
			if h.logger.Debug() {
			...
			// fec.FecLog("sent_packet_handler.go: detectLostPackets(): Case#2 %d is lost (largestAcked=%d)", p.PacketNumber, pnSpace.largestAcked)
			packetLost = true
			if h.logger.Debug() {
		...
		} else if fec.FecEnable && packetLost {
			p.declaredLost = true
			h.removeFromBytesInFlight(p)
			h.queueFramesForRetransmission(p)
	...
	// yunmin added - decrease window size when number of lost packets are greater than the threshold (Raptor decode is failure)
	if fec.FecEnable && err == nil && len(lostPackets) > 2 {
		for _, p := range lostPackets {
			if p.declaredLost {
				...
				if !p.IsPathMTUProbePacket {
					h.congestion.OnPacketLost(p.PacketNumber, p.Length, priorInFlight)
					fec.FecLog("sent_packet_handler.go: detectLostPackets(): Decrease Window Size (Lost packet number=%d)", p.PacketNumber)
				}
			}
		}
	}
...
func (h *sentPacketHandler) ResetForRetry() error {
	...
	h.appDataPackets = newPacketNumberSpace(h.appDataPackets.pns.Pop(), false, h.rttStats) // yunmin added - // yunmin added - change true to false
	oldAlarm := h.alarm
	h.alarm = time.Time{}
	if h.tracer != nil {
		h.tracer.UpdatedPTOCount(0)
