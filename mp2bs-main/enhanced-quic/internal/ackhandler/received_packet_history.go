...
func (h *receivedPacketHistory) GetAckRanges() []wire.AckRange {
	// yunmin added - add FEC receovered packets into ranges
	for _, pn := range fec.FecRecoveredPacketList {
		// fec.FecLog(fmt.Sprintf("received_packet_history.go: GetAckRanges(): FEC recovered packet number=%d", pn))
		h.addToRanges(pn)
	}
	fec.FecRecoveredPacketList = nil // clear fec.FecRecoveredPacketList
	////////////////
